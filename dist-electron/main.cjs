...
function createWindow() {
  ...
  const mainWindow = new electron.BrowserWindow({
    ...
    height: 800,
    webPreferences: {
      preload: path__namespace.join(__dirname$1, "preload.cjs"),
  ...
  electron.ipcMain.handle("start-mcp-server", async (event, serverName, config) => {
    var _a2;
    try {
      ...
      const child = child_process.spawn(resolvedConfig.command, resolvedConfig.args || [], spawnOptions);
      if (!child.pid) {
  ...
  electron.ipcMain.handle("stop-mcp-server", async (event, serverName) => {
    try {
  ...
  electron.ipcMain.handle("get-mcp-server-status", async (event, serverName) => {
    const process2 = mcpServers.get(serverName);
    if (!process2) return "stopped";
    if (process2.killed || process2.exitCode !== null) return "stopped";
    return "running";
  });
  electron.ipcMain.handle("send-mcp-request", async (event, serverName, request) => {
  ...
  electron.ipcMain.handle("fetch-models", async (event, provider) => {
    try {
  ...
  electron.ipcMain.handle("generate-title", async (event, provider, modelId, firstMessage) => {
    var _a2, _b, _c;
    try {
  ...
  electron.ipcMain.handle("cancel-request", async (event, requestId) => {
  ...
  electron.ipcMain.handle("send-message-stream", async (event, provider, modelId, messages, tools, requestId) => {
    var _a2, _b;
  ...
  electron.ipcMain.handle("save-sessions", async (event, sessions) => {
    try {
  ...
  electron.ipcMain.handle("save-projects", async (event, projects) => {
    try {
  ...
  if (((_a = process.env) == null ? void 0 : _a.NODE_ENV) === "development") {
    ...
    const indexPath = path__namespace.join(__dirname$1, "../dist/index.html");
