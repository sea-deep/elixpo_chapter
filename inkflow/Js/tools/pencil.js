class PencilTool {
    ...
    handleMouseUp(e) {
        ...
        if (this.currentElement) {
            this.currentElement.x2 = e.offsetX;
            this.currentElement.y2 = e.offsetY;
            // Final element is already in elements array from mousemove
        }
        this.currentElement = null;
        this.saveState();
        this.redrawCanvas();
    ...
    drawSmoothLine(ctx, x1, y1, x2, y2, options) {
        ctx.beginPath();
        ctx.moveTo(x1, y1);
    ...
    drawElement(element) {
        ...
        this.drawSmoothLine(this.ctx, element.x1, element.y1, element.x2, element.y2, options);
